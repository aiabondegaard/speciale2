...
class DobbeltSUKandidatClass(EconModelClass):
    def settings(self):
    ...
    def s_wage(self,credit,t):
        ...
        #while studying a masters degree and not taking loans
        elif (t<=par.tau) and (par.bachelor<=credit<par.complete):
            return par.su*2.0 + par.s_w*(1-par.tax_s)
        
        #if working
        # if (abs(credit-par.complete))<=0.001:
        #     return par.yl*(1+par.alpha*(credit>=par.complete))
        
        #while studying a bachelors degree and not taking loans
            ...
            for t in range(par.simT):
                ...
                if t<(par.simT-1):
                    ...
                    if  ((sim.G[i,t]<(par.complete)) and (sim.g[i,t]>0.001)) and ((t>par.tau) and (t<=par.t_ls)): 
                        ...
                        sim.income[i,t] = self.wage_func(sim.e[i,t])*sim.psi[i,t] #worker income
