...
class SimpleBreakdownModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        par.gamma_max = 0.00001 #min disutility of studying 
        par.gamma_min = 0.0001 #max disutility of studying
        par.Ngamma = par.Nm
            ...
            for i_e, education in enumerate(par.E_grid):
                for i_a,assets in enumerate(par.a_grid):
                    ...
                    if t==par.T-1: # last period
                        ...
                        res = minimize(obj,c_init,bounds=((lb_c,ub_c),), method='Nelder-Mead')
                ...
                for i_a,assets in enumerate(par.a_grid):
                    for i_G,credit in enumerate(par.G_grid):
                        ...
                        if t>par.t_ls:
                            ...
                            init = np.array([ub_c,guess_g]) if i_a==0 else np.array([sol.c_s[t,i_m,i_a-1,i_G],sol.g[t,i_m,i_a-1,i_G]])
    ...
    def tester(self, motivation, assets, credit, t):
        par = self.par

        obj = lambda x: -self.value_of_choice_s(x[0],x[1],motivation,assets,credit,t)

        lb_c = 0.000001 # avoid dividing with zero
        ub_c = np.maximum(self.s_wage(credit,t)+assets, self.s_wage(credit,t))
        #ub_c = np.maximum(self.s_wage(credit,t)+assets, self.s_wage(credit,t))
        ...
        bounds = ((lb_c,ub_c),(lb_g,ub_g))
        sol = self.sol
        init = np.array([sol.c_s[t-1, 0, 1, list(par.G_grid).index(credit)], sol.g[t-1, 0, 1, list(par.G_grid).index(credit)]])

        x0 = [lb_c, lb_g]

        res = minimize(obj,x0,bounds=bounds,method='Powell')
    ...
    def disutil_study(self,g,motivation,credit):
        ...
        if (credit<par.complete) and (g>0.001):
            return (par.upsilon - ((abs(par.gamma * g))**1.1))
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                idx_sol = (t,sim.e[i,t])
                sim.c[i,t] = interp_1d(par.a_grid,sol.c_w[idx_sol],sim.a[i,t])
